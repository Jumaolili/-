...
var keys = Object.keys(region_index);
...
for (let i = 0; i < input.length; i++) {
    ...
    input[i].addEventListener("click", function () {//添加 pushState  !!!!!!!!!!!!!!!!!!!
        var key;
        for (let i = 0; i < keys.length; i++) {
            if (region_index[keys[i]] == this.value) {
                key = keys[i];
            }
        }
        history.pushState(this.value, null, location.href + "?" + key);

        stateList = location.href.split("?").slice(1);
...
window.addEventListener("popstate", function (e) {
    var checkboxes = document.getElementsByClassName("checkbox");            //后退操作!!!!!!!!!!!!!!!!!!
    var state = e.state;
    

    var str = stateList.pop();  //key
    var value = region_index[str];
    console.log(value);
